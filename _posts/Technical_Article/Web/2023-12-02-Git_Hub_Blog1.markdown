...
toc: true
categories: [Web]
keywords: Jkeyll, Ruby, HTML, SCSS, Java Script, Git Hub Blog , 깃 허브 블로그 제작 시작하기
addsence: true
lastmod: 2024-04-03 15:53:00 +09:00
sitemap:
  changefreq : daily
  priority : 1.0
...
깃 허브 블로그를 만들기 위해서 가장 먼저 해야 할 일은 새로운 레포지토리 생성하는 것입니다.
...
디렉터리를 설치하기 위해서는 원하는 로컬 폴더 주소창에 ＂CMD＂를 사용하여 명령 프롬프트를 열고 명령어를 작성해 줍니다.
...
만약에 리눅스에서 Jekyll을 사용하시면 굳이 루비까지는 설치하실 필요가 없습니다. 리눅스에서는 Jekyll만 설치해도 실행이 됩니다. 하지만 윈도우 운영체제를 사용하시면 Jekyll을 사용하기 위해서 루비를 다운받아야 합니다.
...
루비는 [Ruby 공식사이트](https://www.ruby-lang.org/en/downloads/) 에서 설치하실 수 있습니다. (저 같은 경우에는 설치 당시 최신버전 3.22를 다운 받았습니다.)
...
루비 안에 Jekyll이 있으므로 파이프라인으로 설치해줍니다. Bundler은 루비에서 필요한 gem들의 올바른 버전은 추적하고 설치해서 일관된 환경을 제공해주는 역할을 합니다. 따라서 Jekyll을 올바르게 관리 할 수 있기 때문에 역시 설치해 줘야 합니다.  
...
cmd창에서 위의 명령어를 입력하시면 Jekyll과 bundler을 설치할 수 있습니다.

<br>

### <web_h3>2) Jekyll 블로그 생성 </web_h3>

Jekyll과 bundler을 설치하셨으면, 이제는 Default 블로그를 생성 해야 합니다.
...
블로그 생성하는 명령에서 force를 사용하는 이유는 이미 깃 허브 레파지토리에서 폴더를 생성했기 때문입니다. 따라서 강제로 설치해서 생성하는 옵션입니다.
...
minima테마는 루비를 설치할 때 제공하는 예시 서식입니다. 리눅스에서는 명령어로 간단하게 minima 테마를 옮길 수 있지만, 윈도우버전에서는 루비 폴더에서 찾아서 수동적으로 옮겨야 합니다.
...
위와 같이 루비 폴더에서 minima 테마를 찾으셨다면 복사해서 깃 허브 블로그로 옮기신 후 커밋해봅시다. 여러분의 이제 인터넷 창에 &lt;＂깃 허브 id＂&gt;.github.io 주소로 접속하시면, 여러분 만에 블로그가 하나 생성되었을 것입니다. 커밋한다음 최대 10분 동안 기다려야지 보일 수 있습니다.  
...
처음부터 마음대로 폴더구조와 코드를 작성하신 분들은 참고하지 않으셔도 됩니다. 만약에 Minima 테마를 설치하시고 지워서 활용하시는 분들이라면 이 파트를 보시면 됩니다.  
...
위의 사진 같은 경우에는 저의 테마를 비공개했지만, 만약 여러분이 minima 테마를 설치하셨으면, _config에서 minima를 주석처리 하시면 됩니다.
...
위의 코드를 제거하거나 주석처리 하면, 당연하게 연관돼있는 부분에서 에러가 발생할 것입니다. 발생한 에러의 원인을 찾아서 트레블 슈팅해 봅시다.
...
터미널에 들어가셔서 명령어를 사용해서 jekyll-seo-tag를 설치해줍니다. 명령어로 설치했으면 이제 _config.yml과 Gemfile파일에 seo tag를 정의 해줘야 합니다.
...
에러를 피하고자 버전 관리 할 수 있는 Gemfile에도 seo tag를 정의해주면 트레블 슈팅도 해결입니다.
